<?php
	...
	else if (isset($_POST['usave']))
	{
		...
		else
		{
			...
			$sql = mysqli_query($db,"select * from father_tbl where Password='$old' and Email='$email'");
			if(mysqli_num_rows($sql) > 0)
			{
				if(!empty($npass) && !empty($cpass))
				{
					if($npass == $cpass)
					{
						mysqli_query($db,"update father_tbl set Password='$npass' where Password='$old' and Email='$email'");
	...
	else if(isset($_POST['submit']))
	{
		...
		{
			$sql = mysqli_query($db,"select * from father_tbl where Email='$Username' AND Password='$Password' AND RoleId=1 ");
			...
			if(mysqli_num_rows($sql) > 0)
			{
				...
				mysqli_query($db,"update father_tbl set LastLogin='$datetime' where Email='$email' ");
	...
	function user()
	{
		...
		$mac = $GLOBALS["mac"];
		$sql = mysqli_query($db,"select * from father_tbl where Email='$Username' AND Password='$Password' AND StatusId=1 ");
